...
class NotificationService {
  ...
  Future<void> scheduleNotification(int remainingTasks) async {
    ...
    if (remainingTasks == -1) {
      ...
      message =
          "Get started by adding your first task! 🚀 To add a task, tap the '＋' button and fill in the details. Stay organized and accomplish your goals effortlessly.";
    } else if (remainingTasks == 0) {
      title = 'No Tasks Remaining';
      message =
          'You have completed all your tasks. Great job! Consider adding more tasks to stay productive.';
    } else if (remainingTasks <= 2) {
      title =
          '$remainingTasks ${remainingTasks == 1 ? 'Task' : 'Tasks'} Remaining';
      message =
          'You have $remainingTasks ${remainingTasks == 1 ? 'task' : 'tasks'} left to complete. Keep it up!';
    } else {
      title = 'Urgent: $remainingTasks Tasks Remaining!';
      message =
          'Take your time to work on them steadily and make progress at your own pace!';
    }

    await _fln.periodicallyShow(
        id, title, message, RepeatInterval.daily, _platformSpecific,
